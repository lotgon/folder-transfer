...
function Show-ServeHelp {
  ...
  Write-Host ''
  Write-Host 'COMMON OPTIONS (for the folder form):'
  Write-Host '  -Once               close after the first successful transfer'
  ...
  Write-Host '  folder-transfer.bat D:\data -AllowIp 10.0.0.5 -Once         (single-phase sync)'
  Write-Host '  folder-transfer.bat D:\db   -Cutover                        (two-phase DB sync)'
  Write-Host '  folder-transfer.bat sync.json                              (many folders + ignore, from JSON)'
  Write-Host ''
  Write-Host 'ADVANCED: -Port -IdleSeconds -StallTimeout -NoCompress -NoFirewall -ClientOut -Config'
  Write-Host '  These (and all of the above) can also go in the JSON config. See README.md / sync.example.json.'
  Write-Host ''
  Write-Host 'The token (client secret) is auto-generated and baked into the client - you never set it.'
  Write-Host 'Parameter names are case-insensitive. First argument is a folder OR a .json config.'
  Write-Host ''
}
